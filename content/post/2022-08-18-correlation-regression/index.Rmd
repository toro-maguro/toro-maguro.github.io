...
相関係数も回帰係数も、異なる計算を実行していますが、いずれも何らかの形で線形的な比例関係を捉えようとしているからです。キーは共分散です。
...
**共分散は、下図の+の象限に存在すれば相関係数は正の方に向き、逆もまた然り、という指標です。**
...
したがって、**相関係数は共分散によって平均に対するデータの分布を評価し、xとyの標準偏差によってばらつき方を考慮するというプロセスで、変数間の関連を評価していると表現できます。**
