...
```{r}
# データセットを作る関数
MakeData <- function(n=2000, p_event=0.25, p_exposure=0.5, rr_exposure=2, rr_sex=0.6){
  set.seed(1)
...
* **risk ratio: 1.91 [95%CI: 1.60-2.27]**
* **odds ratio: 2.30 [95%CI: 1.95-3.03]**

**リスク比とオッズ比との乖離がそれなりにありますね**．<br/n>
...
ここで注目していただきたいポイントは信頼区間です．上述のポアソン回帰では，**RR = 2.01 [95%CI: 1.66-2.45]** となりました．<br/n>
一方，2×2表で得たリスク比は **1.91 [95%CI: 1.60-2.27]** であり，**ポアソン回帰で得た信頼区間の幅の方が大きくなっています**．
...
2×2表で得たリスク比は **2.01 [95%CI: 1.70-2.40]** ですから，**通常のポアソン回帰で得たRR = 2.01 [95%CI: 1.66-2.45]よりも狭い信頼区間を得ることができました**．
...
2×2表で得たリスク比は **1.91 [95%CI: 1.60-2.27]** でしたから，普通にできてますね．
